...
import com.m_w_k.gtcefucontent.api.recipes.recipeproperties.LFFRecipeTypeProperty;
import com.m_w_k.gtcefucontent.api.unification.GTCEFuMaterialFlags;
...
public class LinearForgingFurnaceLoader {
    ...
    public static void registerLate() {
        GTCEFuContent.log("Registering Linear Forging Furnace recipes...");
        populateReferences();
    ...
    private static void registerCooled() {
        RecipeMap<BlastRecipeBuilder> map = (RecipeMap<BlastRecipeBuilder>) GTCEFuCRecipeMaps.LINEAR_FORGING_RECIPES[4];
        ...
        map = (RecipeMap<BlastRecipeBuilder>) GTCEFuCRecipeMaps.LINEAR_FORGING_RECIPES[5];
    ...
    private static @Nullable BlastRecipeBuilder produceCooledRecipe(Recipe blastRecipe,
        ...
        var builder = new BlastRecipeBuilder(blastRecipe, targetMap)
                ...
                .duration((int) (Math.ceil(newEU / newEUt) *
                        ConfigHolder.linearForgingFurnaceSettings.coolingDurationModifier));
        ...
        builder.applyProperty(TemperatureProperty.getInstance(), temp +
                ConfigHolder.linearForgingFurnaceSettings.coolingTemperaturePenalty);
        return builder;
    }

    private static void registerForgingCooled() {
        RecipeMap<BlastRecipeBuilder> map = (RecipeMap<BlastRecipeBuilder>) GTCEFuCRecipeMaps.LINEAR_FORGING_RECIPES[7];
        GTRecipeCategory category = GTRecipeCategory.getByName(map.getUnlocalizedName());
        if (category == null)
            throw new IllegalStateException("Component recipemap " + map.getUnlocalizedName() + " had no a category!");
        for (Recipe blastRecipe : RecipeMaps.BLAST_RECIPES.getRecipeList()) {
        ...
        map = (RecipeMap<BlastRecipeBuilder>) GTCEFuCRecipeMaps.LINEAR_FORGING_RECIPES[8];
        category = GTRecipeCategory.getByName(map.getUnlocalizedName());
        if (category == null)
            throw new IllegalStateException("Component recipemap " + map.getUnlocalizedName() + " had no a category!");
        for (Recipe blastRecipe : GCYMRecipeMaps.ALLOY_BLAST_RECIPES.getRecipeList()) {
    ...
    private static List<RecipeBuilder<?>> produceForgingCooledRecipes(Recipe blastRecipe,
        ...
        for (GTRecipeInput input : blastRecipe.getInputs()) {
            ...
            if (stacks != null) {
                ...
                if (mat != null) {
                    if (mat.material.hasFlag(GTCEFuMaterialFlags.NO_FORGING_RECIPES_IN)) return EMPTY_BUILDER_LIST;
                }
            }
        }
        ...
        baseBuilder.duration((int) (baseBuilder.getDuration() *
                ConfigHolder.linearForgingFurnaceSettings.forgingDurationModifier));
    ...
    private static @Nullable RecipeBuilder<?> attemptForgingCooledRecipe(OrePrefix prefix,
        ...
        for (ItemStack stack : baseBuilder.getOutputs()) {
            ...
            var stackPrefix = OreDictUnifier.getPrefix(stack);
            if (mat != null && stackPrefix != null) {
                boolean flag = mat.material.hasFlag(GTCEFuMaterialFlags.NO_FORGING_OUT);
    ...
    public static void assembleCompositeMaps() {
        for (Recipe recipe : RecipeMaps.BLAST_RECIPES.getRecipeList()) {
            ...
            buildToMap(recipe, GTCEFuCRecipeMaps.LINEAR_FORGING_RECIPES[2]);
        }
        ...
        for (Recipe recipe : GTCEFuCRecipeMaps.LINEAR_FORGING_RECIPES[4].getRecipeList()) {
            assignRecipeType(recipe, LinearForgingFurnaceRecipeType.BLAST_COOLED);
            buildToMap(recipe, GTCEFuCRecipeMaps.LINEAR_FORGING_RECIPES[6]);
        }
        for (Recipe recipe : GTCEFuCRecipeMaps.LINEAR_FORGING_RECIPES[5].getRecipeList()) {
            assignRecipeType(recipe, LinearForgingFurnaceRecipeType.ALLOY_COOLED);
            buildToMap(recipe, GTCEFuCRecipeMaps.LINEAR_FORGING_RECIPES[6]);
        }

        for (Recipe recipe : GTCEFuCRecipeMaps.LINEAR_FORGING_RECIPES[7].getRecipeList()) {
            assignRecipeType(recipe, LinearForgingFurnaceRecipeType.BLAST_FORGING_COOLED);
            buildToMap(recipe, GTCEFuCRecipeMaps.LINEAR_FORGING_RECIPES[9]);
        }
        for (Recipe recipe : GTCEFuCRecipeMaps.LINEAR_FORGING_RECIPES[8].getRecipeList()) {
